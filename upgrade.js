...
(()=>{
  ...
  $('installApp').onclick=async()=>{if(installPrompt){await installPrompt.prompt();installPrompt=null;}else{$('installHelp').textContent=location.protocol==='file:'?'이 파일을 HTTPS 웹 주소에 올린 뒤 홈 화면에 추가할 수 있어요. 현재 파일 열기에서는 설치·오프라인 기능을 사용할 수 없습니다.':'Android: 브라우저 메뉴에서 앱 설치 또는 홈 화면에 추가. iPhone: Safari 공유 메뉴에서 홈 화면에 추가를 선택해 주세요. 메뉴 표시는 브라우저마다 다릅니다.';}};
  ...
  showView('home');
  renderUpgrade();
  const requested=new URLSearchParams(location.search).get('date');if(requested){showView('records');$('recordDate').value=requested;$('recordDate').dispatchEvent(new Event('change'));}
