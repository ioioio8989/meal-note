...
window.ME=(()=>{
 ...
 const items=text=>split(text).map(name=>({id:crypto.randomUUID(),name}));
 ...
 function stableItems(old,text){const names=split(text);return names.map((name,i)=>old?.[i]?.name===name?old[i]:{id:crypto.randomUUID(),name});}
 function timeline(date,s=MN.snapshot()){
   const d=s.days[date]||{};
   const enabled=d.institutionEnabled??((s.profile?.institutionMeals||'있음')==='있음'&&!d.home);
   return (enabled?slots:['breakfast','lunch','dinner']).map(slot=>{
   ...
   for(const date of [MN.sessionDate,MN.dateFor('tomorrow')]){
     for(const m of timeline(date,s)){
       ...
       function add(type,ingredient,action){
         ...
         result.push({id,date,slot:m.slot,type,ingredient,prefix,action,menus:title(m.plan.items),done:!!s.prepDone?.[id]});
       }
       for(const name of req.known){
         const available=stock(name,s);
         if(!available.length)add('shop',name,name+' 장보기');
         else if(available.every(x=>x.cat==='냉동')&&/소고기|국거리|양지|삼겹|목살|앞다리|뒷다리|항정|닭|고등어|갈치|가자미|연어|삼치|오징어|새우/.test(available[0].name)){
